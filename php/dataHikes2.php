<?php
    ...
    $name = $_POST["name"];
    $errors = array();
    $dataName = filter_var(strip_tags($name), FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_BACKTICK);

    if (false == filter_var(strip_tags($name), FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_BACKTICK)) {
    ...
    if (false == filter_var($distance, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
        $errors['distance'] = "This distance is invalid.";
        header("Location: ./create.php?message=distanceInvalid");
    }
    ...
    $dataDurationHours = filter_var($durationHours, FILTER_VALIDATE_INT);
    if (false == filter_var($durationHours, FILTER_VALIDATE_INT)) {
    ...
    $dataDurationMinutes = filter_var($durationMinutes, FILTER_VALIDATE_INT);
    if (false ===filter_var($durationMinutes, FILTER_VALIDATE_INT)) {
